...
- 豆包 seed-1.6 系列是思考模型：短任务必须关思考（DoubaoClient 已对 seed 系列自动加 `thinking:{type:"disabled"}`），否则延迟 6s+ 超预算；推荐模型 `doubao-seed-1-6-flash-250615`（实测 <1s）。切换模型前先确认账号已开通（未开通报 ModelNotOpen）。
