...
- **后端**：统一返回 `ApiResponse{code,message,data}`；`X-User-Id` 拦截器 + `UserContext`(ThreadLocal)；AI/语音走 `AiService`/`VoiceService` 接口 + Mock 实现（`gxwy.ai.provider` / `gxwy.voice.provider` 开关预留真实化）；每次 AI 调用写 `ai_request_log`。DTO 用 record（不用 Lombok，JDK25 下注解处理器不稳）；id=UUID 字符串、createdAt=epoch 毫秒。
